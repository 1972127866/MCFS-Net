...
class pSp(nn.Module):
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            ...
            print(f'Loading decoder weights from pretrained path:viton_360t_s_fm.pkl')
            with dnnlib.util.open_url("viton_360t_s_fm.pkl") as f:
                decoder = legacy.load_network_pkl(f)
            self.decoder=decoder['G_ema']
    ...
    def forward(self, x, latent=None, resize=True, latent_mask=None, input_code=False, randomize_noise=True,
        ...
        images,_ = self.decoder.synthesis(codes,noise_mode='random')
        result_latent=codes
