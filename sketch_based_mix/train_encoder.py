...
# 这个文件是训练图像逆映射模块里的encoder，训练的参数在/experiments/emcoder_cloth.json,用--exp引用json文件，如果要用一个新的数据集来训练就复制一个json配置文件写对应的参数
...
def embed_one_example_s_space(args, path, stylegan_encoder, g_all, upsamplers,
                      inter, percept, steps, num_steps, sv_dir, pic_name="-1",
                      skip_exist=False):
    ...
    optimized_latent, optimized_noise, loss_cache = out
    optimized_noise = None
    print("the loss before w opi and after, ", loss_cache[0], loss_cache[-1] )

    # optimized_latent_np = optimized_latent.detach().cpu().numpy()[0]
    # optimized_latent_np=torch.from_numpy(optimized_latent_np).cuda().unsqueeze(0)
    optimized_latent_detached = optimized_latent.detach()

    out = run_embedding_optimization_s_space(args, g_all,
                                     upsamplers, inter, percept,
                                     label_im_tensor, optimized_latent_detached, steps=num_steps,
                                     stylegan_encoder=stylegan_encoder,
                                     use_noise=args['use_noise'],
                                     noise_loss_weight=args['noise_loss_weight']
                                     )
    if args['use_noise']:
        ...
        optimized_styles, optimized_noise, loss_cache = out
        optimized_noise = None
    print("the loss before s opi and after, ", loss_cache[0], loss_cache[-1] )

    # optimized_latent_np = optimized_styles.detach().cpu().numpy()[0]
    optimized_latent_cpu=[]
    for i in range(20):
        optimized_latent_cpu.append(optimized_styles[i].detach().cpu())
    ...
    #                              process_out=True, use_style_latents=True,
    #                              return_only_im=True, noise=optimized_noise)
    img_out, _ = latent_to_image(g_all, upsamplers, latent_in, process_out=True,
                                         dim=args['im_size'][1],
                                         use_style_latents=True, return_only_im=True, noise=optimized_noise, use_style=True,
                                         input_styles=optimized_styles)

    #raw_im_show = (np.transpose(label_im_tensor.cpu().numpy(), (0, 2, 3, 1))) * 255.

    # img_out_pocessed=img_out.cpu().detach().numpy()
    # img_out_pocessed=np.transpose(img_out_pocessed, (0, 2, 3, 1)).astype(np.uint8)
    #vis_list = [im_out_wo_encoder[0],img_out[0]]
    vis_list = [img_out[0]]

    curr_vis = np.concatenate(
        vis_list, 0)
    imageio.imsave(os.path.join(sv_dir, pic_name+".jpg"),
                      curr_vis)

    # imageio.imsave(os.path.join(sv_dir, "real_im.jpg"),
    #                   raw_im_show[0])

    return loss_cache[0], loss_cache[-1], optimized_latent_cpu, loss_cache_np


#test1
def test(args, resume, steps,num_steps,  latent_sv_folder='', skip_exist=False):
    ...
    for i, id in enumerate(tqdm(all_id)):
        ...
        sv_folder = os.path.join(latent_sv_folder,  'w' + str(steps)+'_s' + str(num_steps))

        loss_before_opti, loss_after_opti , all_final_latent, all_final_noise = embed_one_example_s_space(args, all_images[i],
                                                                                                  stylegan_encoder, g_all,
                                                                                                  upsamplers, inter, percept, steps,num_steps,
                                                                                                  sv_folder, skip_exist=skip_exist,pic_name=id)

        all_loss_before_opti.append(loss_before_opti)
        all_loss_after_opti.append(loss_after_opti)

        id_num = id.split("_")[-1]
        latent_name = latent_sv_folder +  '/latents_s_%s.npy' % str(id_num)
        print("latent_name",latent_name)
        print("all_final_latent",len(all_final_latent),all_final_latent[0].shape)
...
if __name__ == '__main__':
    ...
    if args.test:
        test(opts, args.resume, args.steps, args.num_steps,
