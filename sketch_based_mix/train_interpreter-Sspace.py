...
# 这个文件是训练一个可以分割服装图像的网络的，但是应该跟期刊文章的内容没有关系，不用看
...
def prepare_data(args, palette):#把有分割图的图片嵌入stylegan空间，latent_all存储这些图片的潜码文件的路径

    g_all, _, upsamplers, _, avg_latent = prepare_model(args)
    if args['optimized_latent_path']['train'][-4:] == ".npy":
        print("load latent from ",args['optimized_latent_path']['train'])
        ...
        for i in range(args['max_training']):
            ...
            # if i >= 28:
            #     i += 1
            name = 'latents_image_%d.npy' % i

            #im_frame = np.load(os.path.join(args['optimized_latent_path']['train'], name))
            data = np.load(os.path.join(args['optimized_latent_path']['train'], name), allow_pickle=True)
            im_frame = []
            for i in range(20):
                im_frame.append(data[i].cuda())
    ...
    #print("len(latent_all)",len(latent_all))
    ...
    for i in range(len(latent_all)):
        ...
        mask_list.append(mask)#mask_list为mask图像的总list
    ...
    for i in range(len(mask_list)):  # clean up artifacts in the annotation, must do  #len(mask_list)=图片张数
        for target in range(1, 50):
            if (mask_list[i] == target).sum() < 30:#mask_list[i]是一个二维数组，(mask_list[i] == target).sum()计算数组中等于target的元素的个数
                mask_list[i][mask_list[i] == target] = 0


    all_mask = np.stack(mask_list)

    print("next step: initialize all_feature_maps_train and all_mask_train")
    ...
    print("all_feature_maps_train.shape",all_feature_maps_train.shape)
    ...
    for i in range(len(latent_all) ):
        ...
        s_input = latent_all[i]
        #print("latent_input.shape",latent_input.shape)
        #print("img, feature_maps = latent_to_image(...)")
        img, feature_maps = latent_to_image(g_all, upsamplers, None, dim=args['dim'][1],
                                            return_upsampled_layers=True, use_style_latents=args['annotation_data_from_w'],use_style=True,input_styles=s_input)
        # print("img.shape[-2]",img.shape[-2]," feature_maps.shape",feature_maps.shape)
        if args['dim'][0]  != args['dim'][1]:
            # only for car
            img = img[:, 64:448]
            feature_maps = feature_maps[:, :, 64:448]#如果是车，裁剪到384*384
        mask = all_mask[i:i + 1]
        feature_maps = feature_maps.permute(0, 2, 3, 1)#[g的层数*2,特征图维度,特征图维度,总特征维度]

        feature_maps = feature_maps.reshape(-1, args['dim'][2])#[g的层数*2*特征图维度*总特征维度,512]
        # print("feature_maps.permuted&reshaped.shape",feature_maps.shape)
        ...
        curr_vis = np.concatenate( [ img_show, colorize_mask(new_mask, palette)], 0 )
...
def main(args
    ...
    print("prepare_data finish")
    
    print(" all_feature_maps_train_all.shape", all_feature_maps_train_all.shape)
    ...
    print(" for MODEL_NUMBER in range(args['model_num']):")
    for MODEL_NUMBER in range(args['model_num']):
        ...
        classifier = nn.DataParallel(classifier, device_ids=device_ids).cuda()#使用nn.DataParallel函数来用多个GPU来加速训练
