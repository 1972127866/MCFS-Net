...
from mapper.latents_dataset import LatentsDataset
from mapper.textclip_mapper import TextCLIPMapper
from mapper import Ranger
from mapper import train_utils
class Coach:
	def __init__(self, opts):
		...
		self.net = TextCLIPMapper(self.opts).to(self.device)

		# Initialize loss
		# self.id_loss = id_loss.IDLoss(self.opts).to(self.device).eval()#id loss 要删掉。
		...
		#不知道要不要加一个类似skin loss的来限制衣服部分的颜色变化？
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				w, clothstyle_text_inputs, color_text_inputs, selected_description_tuple, hairstyle_tensor, color_tensor = batch
				selected_description = ''
				for item in selected_description_tuple:
					selected_description+=item

				w = w.to(self.device)
				clothstyle_text_inputs = clothstyle_text_inputs.to(self.device)
				...
				with torch.no_grad():
					...
					x,_ = self.net.decoder.synthesis(w)
				#########
				# 简化if else 分支，统一按照没有参考图的路径走
				...
				w_hat = w + 0.1 * self.net.mapper(w, clothstyle_text_inputs, color_text_inputs, hairstyle_tensor_hairmasked, color_tensor_hairmasked)
				#x_hat, w_hat = self.net.decoder([w_hat], input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1)
				x_hat,_, = self.net.decoder.synthesis(w_hat)
				loss, loss_dict = self.calc_loss(w, x, w_hat, x_hat, clothstyle_text_inputs, color_text_inputs, hairstyle_tensor, color_tensor, selected_description)		
	...
	def validate(self):
		...
		for batch_idx, batch in enumerate(self.test_dataloader):
			...
			with torch.no_grad():
				...
				x_hat,_ = self.net.decoder.synthesis(w_hat)
				loss, cur_loss_dict = self.calc_loss(w, x, w_hat, x_hat, clothstyle_text_inputs, color_text_inputs, hairstyle_tensor, color_tensor, selected_description)
		...
		if self.opts.latents_train_path:
			...
			for b in range(self.opts.train_dataset_size // self.opts.batch_size):
				with torch.no_grad():
					...
					_,_, train_latents_b = self.net.decoder.synthesis(train_latents_z[b: b + self.opts.batch_size],return_style=True)
		...
		if self.opts.latents_test_path:
			...
			for b in range(self.opts.test_dataset_size // self.opts.test_batch_size):
				with torch.no_grad():
					...
					# 								  truncation=0.7, truncation_latent=self.net.latent_avg, return_latents=True)
					_,_,test_latents_b = self.net.decoder.synthesis(test_latents_z[b: b + self.opts.test_batch_size],return_style=True)
	...
	def calc_loss(self, w, x, w_hat, x_hat, hairstyle_text_inputs, color_text_inputs, hairstyle_tensor, color_tensor, selected_description):
		loss_dict = {}
		loss = 0.0
		# if self.opts.id_lambda > 0:#id.lambda要设为0
