...
class LatentsDataset(Dataset):
	...
	def __init__(self, latents, opts, status='train'):
		...
		assert (self.opts.clothstyle_manipulation_prob) == 1
		with open(self.opts.clothstyle_description, "r") as fd:
			self.clothstyle_description_list = fd.read().splitlines()

		# self.hairstyle_list = [single_hairstyle_description[:-9] for single_hairstyle_description in self.hairstyle_description_list]
		self.clothstyle_list = [single_clothstyle_description for single_clothstyle_description in self.clothstyle_description_list]
	...
	def manipulate_clothstyle(self, index):
		color_text_embedding = torch.Tensor([0])
		color_tensor = torch.Tensor([0])
		if random.random() < self.opts.clothstyle_text_manipulation_prob:
			selected_clothstyle_description = np.random.choice(self.clothstyle_list)+'top'
			selected_description = selected_clothstyle_description
			clothstyle_text_embedding = torch.cat([clip.tokenize(selected_clothstyle_description)])[0]
			clothstyle_tensor = torch.Tensor([0])
		else:
			clothstyle_text_embedding = torch.Tensor([0])
			img_pil = Image.open(random.choice(self.out_domain_clothstyle_img_path_list))
			clothstyle_tensor = self.image_transform(img_pil)
			selected_description = 'clothstyle_out_domain_ref'
		return self.latents[index], clothstyle_text_embedding, color_text_embedding, selected_description, clothstyle_tensor, color_tensor
	...
	def manipulate_hairstyle(self, index):
		...
		if random.random() < self.opts.hairstyle_text_manipulation_prob:
			...
			hairstyle_text_embedding = torch.cat([clip.tokenize(selected_hairstyle_description)])[0]#这个就是coach里的hairstyle_text_inputs
	...
	def manipulater_cloth_color(self, index):
		clothstyle_text_embedding = torch.Tensor([0])
		clothstyle_tensor = torch.Tensor([0])
		selected_color_description = np.random.choice(self.color_list)+'top'
	...
	def __getitem__(self, index):
		function_list = ['self.manipulate_clothstyle(index)', 'self.manipulater_cloth_color(index)', 'self.manipulater_hairstyle_and_color(index)', 'self.no_editing(index)']
		prob_array = np.array([self.opts.clothstyle_manipulation_prob, self.opts.color_manipulation_prob, self.opts.both_manipulation_prob, (1-self.opts.clothstyle_manipulation_prob-self.opts.color_manipulation_prob-self.opts.both_manipulation_prob)])
