...
if __name__ == "__main__":
    ...
    parser.add_argument('-exclude_trgb', action='store_true')
    parser.add_argument('-img_size', default='256', type=int, help='path to model file')
    ...
    print('Loading networks from "%s"...' % opt.model_path)
    ...
    dlatents =np.load(tmp)[: ,None ,:]#这两行要改，功能是把从文件里得到的w整形
    ...
    #             s_names2.append(tmp)
    # %%
    # layer_num =Gs.components.synthesis.input_shape[1]#14?
    ...
    for m in range(int(opt.num_per)):
        ...
        model_output, affine_layers, watch_s = g_all.module.synthesis( d['model_input:0'], noise_mode='const', return_style=True)
        model_output2 = interpolate(model_output, scale_factor=pool_size)
        assert (model_output2.shape[-1] == out_size)
        watch_s_size = []
        for t in watch_s:
            tmp =t.get_shape().as_list()
        ...
        d['G_synthesis_2/images_out:0'] = model_output

        watch_s_value = []
        for i in range(20):
            if opt.exclude_trgb and (i+2)%3==0:
                continue
            tmp = d['orig_s_'+str(i)]
            watch_s_value.append(tmp)

        var_grad = compute_gradient2(x=watch_s, x_shape=watch_s_size, y=model_output2,
                                     y_shape=(1, 3, out_size, out_size),
                                     x_init_value=watch_s_value, extra_feed_dict=d)
        for k in range(20):
            if opt.exclude_trgb and (k+2)%3==0:
                continue
