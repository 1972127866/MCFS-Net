...
def creatPics(opt,lc_list):
    ...
    for picid in tqdm(range(int(opt.pic_num)),position=0):
        ...
        PIL.Image.fromarray(img1[0].cpu().numpy(), 'RGB').save(
            os.path.join(opt.output_path, "class_"+opt.sema_inx, str(picid) + "_orig.jpg"))
        ...
        for j in range(len(lc_list)):
            ...
            PIL.Image.fromarray(img2[0].cpu().numpy(), 'RGB').save(
                os.path.join(save_path,str(picid)+"_pos"+str(round(ss[layer][0][channel].item()))+".jpg"))
            ...
            PIL.Image.fromarray(img3[0].cpu().numpy(), 'RGB').save(
                os.path.join(save_path,str(picid)+"_neg"+str(round(ss[layer][0][channel].item()))+".jpg"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-output_path', default='../../hdd/wanqing/editGAN', type=str,
                        help='path to ')
