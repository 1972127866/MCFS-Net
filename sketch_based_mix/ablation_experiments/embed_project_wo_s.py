...
def project(
    ...
    *,
    ...
    for i in range(len(s_samples)):
        ...
        # print("s_sample[i.shape]",s_sample[i].shape)
        # print("s_avg[i].shape",s_avg[i].shape)
        # print("type(s_sample[i])",type(s_sample[i]))
        # print("type(s_avg[i])",type(s_avg[i]))
        # print("np.sum test",np.sum(s_sample[i] - s_avg[i])** 2)
...
#         all_final_latent = all_final_latent.unsqueeze(0)
        ...
        synth_image = (synth_image + 1) * (255 / 2)
        synth_image = synth_image.permute(0, 2, 3, 1).clamp(0, 255).to(torch.uint8)[0].cpu().numpy()
        PIL.Image.fromarray(synth_image, 'RGB').save(f'{sv_folder}/%s_embed_wo_s.png'%str(id))
        s_cpu=[]
        for i in range(20):
            s_cpu.append(all_styles[i].cpu())
        np.save(f'{sv_folder}/%s_embed_wo_s.npy'%str(id),s_cpu)
