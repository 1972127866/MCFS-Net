...
from datasets.test_dataset_texture_avg_all_s_text_control import TestLatentsDataset
...
def main(opts):
    ...
    target_name=opts.target.replace(' ', '-').replace(',', '_')

    os.makedirs(os.path.join(opts.save_dir,"text_control","use_avg_all_s_extra_mapper_img_loss",opts.images_classname), exist_ok=True)
    # print(target_list)
    ...
    for bid, batch in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            torchvision.utils.save_image(img_gen_all, os.path.join(opts.save_dir,"text_control","use_avg_all_s_extra_mapper_img_loss",opts.images_classname,target_name+"_%04d.jpg" %(bid+1)), normalize=True, range=(-1, 1))
