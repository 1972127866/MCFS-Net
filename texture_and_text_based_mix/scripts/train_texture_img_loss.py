...
from datasets.train_dataset_texture_all_s import TrainLatentsDataset #用all_s的dataloader也可以加载非all_s的数据
...
import torchvision
from utils.texture_loss import TextureLoss
...
def main(opts):
    ...
    #save dir
    if opts.resume is not None:
        os.makedirs(os.path.join(opts.checkpoint_path, "img_loss","resume",opts.images_classname,opts.texture_classname), exist_ok=True)
    else:
        os.makedirs(os.path.join(opts.checkpoint_path, "img_loss",opts.images_classname,opts.texture_classname), exist_ok=True)
#     os.makedirs(os.path.join(opts.checkpoint_path,"img_val"), exist_ok=True)
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        img_ori = stylespace_util.decoder_validate(g_ema, latent_s + delta_s, latent_s)
        img_fake=stylespace_util.decoder_validate(g_ema, latent_s + fake_delta_s, latent_s)
...
#         torchvision.utils.save_image(img_ori,os.path.join("./checkpoints", "img_val","real.jpg"))
        ...
        loss_texture = texture_loss.get_texture_loss(img_ori,img_fake)

        loss = opts.l2_lambda * loss_l2 + opts.cos_lambda * loss_cos + loss_texture
        loss.backward()
        optimizer.step()

        if batch_idx % opts.print_interval == 0 :
            print(batch_idx, loss.detach().cpu().numpy(), loss_l2.detach().cpu().numpy(), loss_cos.detach().cpu().numpy(),loss_texture.detach().cpu().numpy())
        ...
        if batch_idx % opts.save_interval == 0:
            if opts.modi_loss or opts.only_medium:
               ...
               assert 0==1
                ...
                if opts.resume is not None:
                    torch.save(net.state_dict(), os.path.join(opts.checkpoint_path, "img_loss","resume",opts.images_classname,opts.texture_classname,"net_%06d.pth" % batch_idx))
                else:
                    torch.save(net.state_dict(), os.path.join(opts.checkpoint_path, "img_loss",opts.images_classname,opts.texture_classname,"net_%06d.pth" % batch_idx))
