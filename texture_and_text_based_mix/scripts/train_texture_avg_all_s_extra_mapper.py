...
def main(opts):
    ...
    #save dir
    os.makedirs(os.path.join(opts.checkpoint_path, "avg_all_s_extra_mapper",opts.images_classname,opts.texture_classname), exist_ok=True)
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        if batch_idx % opts.save_interval == 0:
            if opts.modi_loss:
                torch.save(net.state_dict(), os.path.join(opts.checkpoint_path, "avg_modi_loss",opts.images_classname,opts.texture_classname,"net_%06d.pth" % batch_idx))
            elif opts.only_medium:
                ...
                torch.save(net.state_dict(), os.path.join(opts.checkpoint_path, "avg_all_s_extra_mapper",opts.images_classname,opts.texture_classname,"net_%06d.pth" % batch_idx))
