...
def main(opts):
    ...
    os.makedirs(os.path.join(opts.save_dir,opts.texture_classname,"use_avg_all_s_extra_mapper_img_loss",opts.images_classname), exist_ok=True)
    ...
    for bid, batch in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            # img_gen_all = torch.cat(img_list, dim=3)
                torchvision.utils.save_image(img_gen, os.path.join(opts.save_dir,opts.texture_classname,"use_avg_all_s_extra_mapper_img_loss",opts.images_classname, "%04d.jpg" %(bid+1)), normalize=True, range=(-1, 1))
