...
from datasets.test_dataset_texture_all_s import TestLatentsDataset
...
def main(opts):
    ...
    g_ema = Generator(size=opts.stylegan_size, style_dim=512, n_mlp=2)
    ...
    os.makedirs(os.path.join(opts.save_dir,opts.texture_classname,"use_all_s_extra_mapper_img_loss",opts.images_classname), exist_ok=True)
    ...
    for bid, batch in enumerate(test_dataloader):
        ...
        latent_s = latent_s.to(device) # [1, 7424]
        delta_c = delta_c.to(device) #[1, 1024]
        ...
        with torch.no_grad():
            ...
            # img_gen_all = torch.cat(img_list, dim=3)
                torchvision.utils.save_image(img_gen, os.path.join(opts.save_dir,opts.texture_classname,"use_all_s_extra_mapper_img_loss",opts.images_classname, "%04d.jpg" %(bid+1)), normalize=True, range=(-1, 1))
