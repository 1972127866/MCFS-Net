...
def main(opts,t_id):
    ...
    if opts.texture_ids[0]!=-1:
        ...
        for image_path in tqdm(path_list):#texture
            count+=1
            if count>=t_id and count<t_id+100:
    ...
    for texture_index, delta_c in tqdm(enumerate(delta_c_list)): #t0s0,t0s1,t0s2,t1s0,t1s1,t1s2
        ...
        with torch.no_grad():
            ...
            if opts.target_text is not None:
                ...
                torchvision.utils.save_image(img_gen, os.path.join(opts.save_path,opts.save_name, str(t_id+texture_index//len(opts.cloth_ids))+"_"+str(cloth_index)+".jpg"), normalize=True, range=(-1, 1))
