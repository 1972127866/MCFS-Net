...
def main(opts):
    ...
    #save dir
    if opts.resume is not None:
        os.makedirs(os.path.join(opts.checkpoint_path, "all_s_img_loss","resume",opts.images_classname,opts.texture_classname), exist_ok=True)
    else:
        os.makedirs(os.path.join(opts.checkpoint_path, "all_s_img_loss",opts.images_classname,opts.texture_classname), exist_ok=True)
        ...
        if batch_idx % opts.save_interval == 0:
            if opts.modi_loss or opts.only_medium:
               ...
               assert 0==1
                ...
                if opts.resume is not None:
                    torch.save(net.state_dict(), os.path.join(opts.checkpoint_path, "all_s_img_loss","resume",opts.images_classname,opts.texture_classname,"net_%06d.pth" % batch_idx))
                else:
                    torch.save(net.state_dict(), os.path.join(opts.checkpoint_path, "all_s_img_loss",opts.images_classname,opts.texture_classname,"net_%06d.pth" % batch_idx))
