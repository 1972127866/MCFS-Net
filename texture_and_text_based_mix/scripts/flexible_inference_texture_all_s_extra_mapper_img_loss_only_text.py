...
def main(opts):
    ...
    for num in tqdm(range(len(opts.cloth_ids)),position=0):
        ...
        delta_c = torch.cat([ori_texture_c[0], ori_texture_c[0]], dim=0).unsqueeze(0)
    ...
    if opts.target_text is not None:
        ...
        neutral=neutral.replace('_',' ')
        targets=opts.target_text.replace('_',' ')
        ...
        for target in target_list:
            ...
            dt = dt / dt.norm(dim=-1, keepdim=True).float().clamp(min=1e-5)#[512]
            dt_list.append(dt)

    print("generating result")
    for cloth_index in tqdm(range(len(s_latents_list))): #t0s0,t0s1,t0s2,t1s0,t1s1,t1s2
        latent_s=s_latents_list[cloth_index]
        delta_c=delta_c_list[cloth_index]
        ...
        with torch.no_grad():
            if opts.target_text is not None:
                for text_index, dt in enumerate(dt_list):
                    ...
                    img_gen = stylespace_util_all_s.decoder_validate(g_ema, latent_s +  fake_delta_s_text, batch_size_latent)
                    torchvision.utils.save_image(img_gen, os.path.join(opts.save_path,opts.save_name, target_list[text_index]+"_"+str(cloth_index)+".jpg"), normalize=True, range=(-1, 1))
                    s_cpu=[]
                    s_out=latent_s + fake_delta_s_text
                    s_out=stylespace_util_all_s.split_stylespace_256(s_out)
                    for i in range(20):
                        s_cpu.append(s_out[i].cpu())
                    np.save(os.path.join(opts.save_path,opts.save_name, target_list[text_index]+"_"+str(cloth_index)+".npy"),s_cpu)
            else:
                img_gen = stylespace_util_all_s.decoder_validate(g_ema, latent_s, batch_size_latent)
                torchvision.utils.save_image(img_gen, os.path.join(opts.save_path,opts.save_name,str(cloth_index)+".jpg"), normalize=True, range=(-1, 1))
