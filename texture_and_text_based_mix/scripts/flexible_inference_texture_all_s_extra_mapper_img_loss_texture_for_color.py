        ...
        with torch.no_grad():
            ...
            if opts.target_text is not None:
                for text_index, dt in enumerate(dt_list):
                    ...
                    # 下面的置零与flexible_inference_texture_all_s_extra_mapper_img_loss.py中相反
                    #coarse
                    delta_s[:, :4*512]=0
                    fake_delta_s_text[:, :4*512]=0
                    #medium
                    delta_s[:, 5*512:7*512]=0
                    delta_s[:, 8*512:10*512]=0
                    #fine
                    delta_s[:, 11*512:12*512+256]=0
                    delta_s[:, 12*512+256*2 : 12*512 + 256*3 + 128]=0
                    delta_s[:, 12*512 + 256*3 + 128*2 : 12*512 + 256*3+ 128*3 + 64]=0
                    # rgb
