...
class DeltaMapper(Module):
    ...
    def __init__(self):
        ...
        self.fm_fine   = Mapper(1792*2, 1792, norm=False)
        
    def forward(self, sspace_feat, clip_feat):

        s_coarse = sspace_feat[:, :4*512].view(-1,4,512)
        ...
        x_medium = torch.cat([s_medium, torch.stack([c_medium]*7, dim=1)], dim=2) #[b,4,1024]
        ...
        x_medium = x_medium.view(-1,7*512)
